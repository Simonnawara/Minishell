...
/*   Updated: 2025/01/27 15:59:41 by sinawara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_echo(char **args, char **res, int echo_counter, t_exec *exec)
{
		...
		&& res[res_index][0] != '<')
	{
		...
		if (ft_strchr(stripped_arg, '$'))
		{
			...
			processed_arg = get_command(res[res_index], total_quotes,
					quote_type);
			if (!processed_arg)
			{
				free(stripped_arg);
				...
				&& ft_strchr(processed_arg, '$'))
			{
				...
				else
				{
					...
					if (!expanded_arg)
					{
						cleanup_echo_resources(stripped_arg, processed_arg,
							NULL);
						return (1);
					}
		...
		if (res[res_index + 1] && res[res_index + 1][0] != '>' && res[res_index
			+ 1][0] != '<' && !prev_was_quote && !next_is_quote
			&& !curr_is_quote)
			ft_putstr_fd(" ", 1);
